...
RSpec.describe CharacterUpdaterJob do
  describe "#perform" do
    it "calls character updater and broadcaster" do
      ...
      allow(CharacterUpdater).to receive(:call).with(character) { character }
    ...
    it "handles Armory errors" do
      ...
      allow(CharacterUpdater).to receive(:call).and_raise(Armory::ServerError)
    ...
    it "handles uniqueness errors" do
      character = instance_double("Character", updated_at: Time.current)
      allow(CharacterUpdater).to receive(:call).and_raise(ActiveRecord::RecordNotUnique)
